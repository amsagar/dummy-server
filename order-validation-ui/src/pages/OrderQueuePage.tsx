...
import type { OverallStatus, ResultStatus, RunSource } from "@/types/orderValidation";
...
const SOURCE_LABELS: Record<RunSource, { label: string; cls: string }> = {
  AGENT: { label: "Agent", cls: "bg-pods-blue/10 text-pods-blue" },
  QUICK_TEST: { label: "Quick test", cls: "bg-muted text-muted-foreground" },
  INGESTED: { label: "Ingested", cls: "bg-amber-500/10 text-amber-600" },
};
function sourcePill(source: RunSource | null | undefined) {
  ...
  return (
    <span className={cn("rounded-md px-1.5 py-0.5 text-[10px] font-medium", meta.cls)}>
      {meta.label}
    </span>
  );
}
