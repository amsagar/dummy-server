...
// Dedicated OV API path. The backend mounts the same chat / sessions
// handlers at both /api/v1 and /api/v1/order-validation; we use the OV
// path so the OvScopeInterceptor attaches the scoped allow-list to
// every request. Result: this UI's assistant only sees the configured
// subset of skills, rule-domains, and decision tables.
