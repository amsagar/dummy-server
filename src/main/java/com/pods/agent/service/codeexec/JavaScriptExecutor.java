...
public class JavaScriptExecutor implements CodeExecutor {
    ...
    public CodeExecutionResult execute(String code, Map<String, Object> input, int memoryLimitMb) {
        ...
        String encodedCode = Base64.getEncoder().encodeToString((code == null ? "" : code).getBytes(StandardCharsets.UTF_8));
        String encodedCodeLiteral = processSupport.jsonLiteral(encodedCode);
        String wrapper = """
                const fs = require('fs');
                const raw = fs.readFileSync(0, 'utf8');
                const input = raw ? JSON.parse(raw) : {};
                const __pods_code = Buffer.from(%s, 'base64').toString('utf8');
                ...
                """.formatted(encodedCodeLiteral, marker);
